...
def permute_sequence(sequence, type , ratio ):
    sequence = list(sequence)
    length = len(sequence)
    num_nucleotides = max(1, int(ratio * length))
    ...
    def insert(sequence):
        ...
        sequence.insert(pos, nucleotide)

    def duplicate(sequence):
        start = random.randint(0, len(sequence) - 1)
        end = min(len(sequence), start + random.randint(1, 10))  
        segment = sequence[start:end]
        insert_pos = random.randint(0, len(sequence))
        sequence[insert_pos:insert_pos] = segment

    def substitute(sequence):
        if sequence:
            pos = random.randint(0, len(sequence) - 1)
            original = sequence[pos]
            substitutes = [nuc for nuc in 'ACGT' if nuc != original]
            sequence[pos] = random.choice(substitutes)
    ...
    if type == 'Deletion':
        ...
        for _ in range(num_nucleotides):
            mutation_type = random.choice(['Deletion', 'Insertion', 'Duplication', 'Substitution'])
...
def val_dataset_generator(
    ...
    for file in os.listdir(val_dir):
        ...
        for seq, label in zip(df_test["SEQ"], df_test[cls]):
            if ratio > 0:
                seq = permute_sequence(seq, type = perturbation, ratio = ratio)
